...
// ============================================================
function preload() {
  // Load the punch sound once and fill the array with it
  let punchSound = loadSound("assets/sounds/SlapSound.wav");
  for (let i = 1; i <= 9; i++) {
    punchSounds.push(punchSound);
  }
  winSound = loadSound("assets/sounds/win.wav");
  bgMusic = loadSound("assets/sounds/demonmusic.mp3");
  bgImage = loadImage("assets/images/portal.jpeg");
}
...
function drawStartScreen() {
  ...
  textAlign(CENTER);
  textSize(52);
  text("PORTAL BALL FIGHT", width / 2, height / 2 - 20);

  // Subtitle
  fill(0);
  textSize(18);
  text("Enter the portal and battle it out!", width / 2, height / 2 + 20);

  // Controls — each player shown in their colour
  textSize(14);
  fill(0, 200, 180);
  text("P1: A/D move   F attack   G block", width / 2, height / 2 + 70);
  fill(255, 150, 30);
  text("P2: Arrows move   K attack   L block", width / 2, height / 2 + 95);
...
function drawWinScreen() {
  ...
  textAlign(CENTER);
  textSize(40);
  text(winner + " ESCAPED THE PORTAL REALM!", width / 2, height / 2 - 30);
